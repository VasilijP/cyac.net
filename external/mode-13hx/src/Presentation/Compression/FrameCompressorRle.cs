...
public sealed class FrameCompressorRle(int offset = 0, int count = 0) : IFrameCompressor
{
    public const int PixelsPerBlock = 63; // optimum for CPU is around 63 (for 64x4 byte block) but GPU is happier with larger number of blocks
    ...
    public string DecompressShaderName => "decomp_rle.comp";
    ...
    public void Setup(FrameDescriptor fd, int compressorDesiredCount)
    {
        ...
        for (int i = 0; i < fd.Compressor.Length; ++i) { fd.Compressor[i] = new FrameCompressorRle(i * cbs*fd.Buffer.Height, (cbs-0)*fd.Buffer.Height); }; // create compressor collection per frame descriptor (it is possible to compress multiple frames in parallel)
